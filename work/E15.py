...
# dt = datetime(2021, 4, 29)
dt_epoch = create_epoch([datetime(2023, 3, 14), datetime(2023, 3, 20)], timedelta(hours=24))

# dt_epoch = sub_wispr_epoch
fov_dt = datetime(2023, 3, 17, 20)
...
plot = go.Figure(data=[plot_Spacecraft_model('SPP', fov_dt, scale=5,color='silver',lighting=dict(specular=2))]
                      ...
                      #                       delta_deg=3.,
                      #                       deg_vs_r_vect=1. - 1. / (
                      #                               abs(np.arange(track['r0'], 25., .5) - track['r0']-5.)
                      #                               * abs(np.arange(track['r0'], 25., .5) - track['r0'] - 8.) + 1)
                      ...
                      #                       delta_deg=3.,
                      #                       deg_vs_r_vect=1. - 1. / (
                      #                               abs(np.arange(track['r0'], 1., -.5) - track['r0']-5.)
                      #                               * abs(np.arange(track['r0'], 1., -.5) - track['r0'] - 8.) + 1)
                      #
                      #                       # deg_vs_r_vect=np.log(
                      ...
                      + [plot_object_orbit('SPP', dt_epoch, type='line', color='white', width=6),
                         plot_object_orbit('SPP',dt_epoch,type='marker',color='white'),
                         plot_HCS(crid, 'corona', **surface_dict,lighting=dict(diffuse=0.9)),
                         plot_HCS(crid, 'helio', **surface_dict,lighting=dict(diffuse=0.9)),
                         plot_sun(crid, scale=1.5)],
                 layout=go.Layout(scene=dict(xaxis_title='X (Rs)', yaxis_title='Y (Rs)', zaxis_title='Z (Rs)',
                                             xaxis_range=[-50, 50], yaxis_range=[-50, 50], zaxis_range=[-50, 50],
