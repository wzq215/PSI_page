...
dt_epoch = create_epoch([datetime(2022,9,10),datetime(2022,9,30)],timedelta(hours=3))

plot = go.Figure(data = [plot_Spacecraft_model('SOLO', datetime(2022,9,10), scale=50,color='silver'),
                         plot_planet_model('earth',datetime(2022,9,10),radius=10)]
                        +sum([plot_flux_tube_from_object('SOLO', datetime(2022,9,10),color='white')],[])
                        +sum([plot_flux_tube_from_object('earth', datetime(2022,9,10),color='white')],[])
