...
def plot_HCS(crid, region, surface_dict):
    ...
    r = np.array(data['scales1'])  # in Rs, distance from sun
    t = np.array(data['scales2'])  # in rad, latitude
    p = np.array(data['scales3'])  # in rad, Carrington longitude
...
def plot_obj_magtrace(obj_str, dt_epoch, stop_at_obj=True):
    et_epoch = spice.datetime2et(dt_epoch)
    ...
    for i in range(len(dt_epoch)):
        ...
        tmp_mag_trace = PSI_trace(obj_pos[:, i], crid)
        trace_points = []
        for mag_trace in tmp_mag_trace:
        ...
        SC_magtrace.append(go.Scatter3d(x=trace_points[:, 0], y=trace_points[:, 1], z=trace_points[:, 2],
...
def get_SC_traces(SC_dict):
    ...
    if SC_dict['model_position_dt'] != None:
        traces_model = plot_Spacecraft_model(SC_str, SC_dict['model_position_dt'])
        for trace in traces_model:
            SC_trace_list.append(trace)
    if len(SC_dict['trace_epoch']) != 0:
        traces_trace = plot_obj_magtrace(SC_str, SC_dict['trace_epoch'])
        for trace in traces_trace:
            SC_trace_list.append(trace)
